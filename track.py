...
class Track:
    ...
    @classmethod
    def generate(cls, n_points: int = 400, seed: Optional[int] = None) -> 'Track':
        rng = np.random.default_rng(seed)
        ...
        for i in range(num_features):
            for _ in range(10):
                ...
                func = rng.choice(feature_funcs, p=[0.4, 0.3, 0.3])
    ...
    # ++ NEW: Load from JSON function ++
    @classmethod
    def load_from_json(cls, file_path: str) -> 'Track':
        """Loads a track from a JSON file containing reconstruction data."""
        with open(file_path, 'r') as f:
            data = json.load(f)
        
        if 'reconstruction_data' not in data:
            raise ValueError("JSON file does not contain 'reconstruction_data'.")
            
        recon_data = data['reconstruction_data']
        control_points = np.array(recon_data['control_points'])
        pit_mask = np.array(recon_data['pit_mask'])
    ...
    def export_to_json(self, file_path: str, num_samples: int = 1000):
        ...
        print(f"Exported full track data to '{file_path}'")
